...
class BidirectionalFoodSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                nextFood = state[1].copy()
                if (next_x,next_y) in nextFood:
                    nextFood.remove((next_x,next_y))
                successors.append((((next_x, next_y), nextFood), direction, 1))
        return successors
    ...
    def getBackwardsSuccessors(self, state):
        ...
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[next_x][next_y]:
                nextFoodGrid = Grid(self.foodGrid.width, self.foodGrid.height)

                for (x1,y1) in state[1]:
                    nextFoodGrid[x1][y1]=True
                nextFoodGrid[x][y]=self.foodGrid[x][y]
                nextFood=nextFoodGrid.asList()
                rev_action = Actions.reverseDirection(direction)
                successors.append( ( ((next_x, next_y), nextFood),  rev_action, 1) )
        return successors
